...
Issues and specs for this repo live as GitHub issues on `Samuel-Windersvista/SamMeow-Inescapable-Tarkovs-Softcore`. Use the `gh` CLI for all operations.

> 本地快照：规格书的可读快照同时随仓库归档于 `docs/specs/`（以 GitHub issue 为准；快照仅用于离线浏览与版本化）。
...
- **Blocking**: GitHub's native issue dependencies (`gh api --method POST repos/<owner>/<repo>/issues/<child>/dependencies/blocked_by -F issue_id=<blocker-db-id>`). Fall back to a `Blocked by: #<n>` line at the top of the child body when unavailable.
